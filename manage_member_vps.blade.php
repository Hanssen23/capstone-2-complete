<x-layout>
    ...
    <script>
        ...
        function showReceiptPreview() {
            ...
            // Create modal with semi-transparent background overlay
        ...
        async function processPayment() {
            ...
            if (membershipCheck.has_active_plan) {
                console.log('Active plan found, automatically overriding');
                // Close the receipt preview modal first - force immediate close
                ...
                setTimeout(() => {
                    // Show confirmation modal for membership override
        ...
        function updateMemberListDisplay() {
            ...
            const filteredMembers = memberListData.filter(member => {
                // Exclude inactive members (status check only - hard delete is now used)
                if (member.status === 'deleted') {
                    return false;
                }

                if (!searchTerm) return true;
                const searchText = `${member.full_name} ${member.email} ${member.member_number}`.toLowerCase();
        ...
        function createMemberCard(member) {
            ...
            memberCard.className = 'member-card bg-white border rounded-lg p-4 sm:p-6 cursor-pointer transition-all duration-200 hover:shadow-md';
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div class="flex-1">
                        ...
                        <div class="text-sm text-gray-600 space-y-1">
                            <div><strong>Member #:</strong> ${member.member_number}</div>
                            <div><strong>Email:</strong> ${member.email}</div>
                            <div><strong>Phone:</strong> ${member.mobile_number || 'N/A'}</div>
                        </div>
                    </div>
