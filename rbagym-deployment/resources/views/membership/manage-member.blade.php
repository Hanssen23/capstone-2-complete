<x-layout>
    ...
    <script>
        ...
        function selectMember(cardElement) {
            ...
            // Check for active membership before showing plan selection
            checkActiveMembership(selectedMember.id);
        }

        // Check if member has active membership
        function checkActiveMembership(memberId) {
            fetch('{{ route("membership.check-active-membership") }}', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.has_active_plan) {
                    // Show warning about active membership
                    showActiveMembershipWarning(data);
                } else {
            ...
            .catch(error => {
                ...
                // On error, still show plan selection form
                showPlanSelectionForm();
            });
        }
        ...
        function showActiveMembershipWarning(membershipData) {
            const warningHtml = `
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-6 mb-6">
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            <h3 class="text-sm font-medium text-yellow-800">Active Membership Found</h3>
                            <div class="mt-2 text-sm text-yellow-700">
                                <p>This member already has an active membership plan:</p>
                                <ul class="list-disc list-inside mt-1">
                                    <li><strong>Plan:</strong> ${membershipData.plan_name}</li>
                                    <li><strong>Started:</strong> ${membershipData.start_date}</li>
                                    <li><strong>Expires:</strong> ${membershipData.expiration_date}</li>
                                </ul>
                                <p class="mt-2">You cannot process a new payment while an active membership exists.</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            // Insert warning before plan selection form
            const planSelectionForm = document.getElementById('planSelectionForm');
            planSelectionForm.innerHTML = warningHtml;
            planSelectionForm.classList.remove('hidden');
        }
        ...
        function showReceiptPreview() {
            ...
            // Create modal with lighter backdrop
            const modal = document.createElement('div');
            modal.className = 'fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4';
            modal.innerHTML = `
                <div class="bg-white rounded-xl shadow-lg max-w-md w-full max-h-[90vh] overflow-auto transform transition-all duration-300 scale-95 opacity-0 border border-gray-200"
                    ...
                    <!-- Receipt Content -->
                    <div class="p-4">
                        <!-- Header Section with Logo and Amount -->
                        <div class="text-center mb-4">
                            <img src="{{ asset('images/rba-logo/rba logo.png') }}" alt="RBA Logo" class="w-16 h-auto mx-auto mb-2">
                            <h2 class="text-lg font-bold text-gray-900 mb-1">Ripped Body Anytime</h2>
                            <p class="text-xs text-gray-600 leading-tight mb-3">
                                Blk. 168 Deparo, City of Caloocan,<br>
                                Caloocan, Philippines, 1400
                            </p>
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                                <p class="text-xs text-gray-500 uppercase tracking-wide mb-1">Receipt #Preview</p>
                                <p class="text-2xl font-bold text-blue-600">₱${finalAmount.toFixed(2)}</p>
                                <p class="text-xs text-gray-500">VIP Membership | Annually (1yr)</p>
                            </div>
                        </div>

                        <!-- Payment Details Section -->
                        <div class="mb-4">
                            <h4 class="text-sm font-semibold text-gray-900 mb-2 border-b border-gray-200 pb-1">Payment Details</h4>
                            <div class="space-y-1">
                                <div class="flex justify-between text-xs">
                                    <span class="text-gray-600">Date:</span>
                                    <span class="font-medium text-gray-900">${new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                </div>
                                <div class="flex justify-between text-xs">
                                    <span class="text-gray-600">Time:</span>
                                    <span class="font-medium text-gray-900">${new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true })}</span>
                                </div>
                                <div class="flex justify-between text-xs">
                        ...
                        <div class="mb-4">
                            <h4 class="text-sm font-semibold text-gray-900 mb-2 border-b border-gray-200 pb-1">Membership Details</h4>
                            <div class="space-y-1">
                                <div class="flex justify-between text-xs">
                                ...
                                <div class="flex justify-between text-xs">
                                    ...
                                    <span class="font-medium text-gray-900">${new Date(startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                </div>
                                <div class="flex justify-between text-xs">
                                    <span class="text-gray-600">Expiration Date:</span>
                                    <span class="font-medium text-gray-900">${expiration.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mb-4">
                            <h4 class="text-sm font-semibold text-gray-900 mb-2 border-b border-gray-200 pb-1">Payment Summary</h4>
                            <div class="space-y-1">
                                <div class="flex justify-between text-xs">
                                ...
                                ` : ''}
                                <div class="flex justify-between text-xs font-semibold border-t border-gray-200 pt-1">
                        ...
                        <div class="text-center border-t border-gray-200 pt-3">
                            <p class="text-xs text-gray-600 mb-2">Thank you for choosing Ripped Body Anytime!</p>
                            <p class="text-xs text-gray-500">Generated on ${new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                        </div>
                    </div>

                    <!-- Modal Footer -->
                    <div class="p-3 border-t border-gray-200 bg-gray-50">
                        <button onclick="processPayment()" class="w-full px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-all duration-200 font-medium text-sm">
